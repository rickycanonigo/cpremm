      ...
      <div>
        ...
        <nav class="navbar fixed-top navbar-expand-lg navbar-light bg-light" 
          ...
          <div class="container-fluid" style={{backgroundColor:'white'}}>
            <Collapse id="topnav" isOpen={!this.state.collapsed} navbar>
              <ul className="navbar-nav mr-auto">
                {
                  this.state.appRoutes.map((appRoute, i) => {
                    return (
                      // <Fragment>
                        <UncontrolledDropdown nav inNavbar size="sm" className={((appRoute.active)?" active":"") + ((appRoute.hasOwnProperty("sub"))?(" withSubmenu" + ((appRoute.isOpen)?" submenu-open":" submenu-close")):"")}>
                          {
                            (appRoute.hasOwnProperty("sub"))
                              ...
                              :<DropdownToggle  nav onClick={this.toggleNavbar}>
                                <NavLink to={appRoute.path} isActive={oddEvent} 
                                  onClick={async () => { 
                                    await this.toggleRoute(appRoute.path);
                                    await this.refreshMenusActiveSubmenu();
                          ...
                          }
                          {
                            (appRoute.hasOwnProperty("sub"))
                              ?
                              <DropdownMenu isOpen={appRoute.isOpen} right onClick={this.toggleNavbar}>
                                {
                                  appRoute.sub.map((sublink) => {
                                      return(
                                        <NavLink to={sublink.path} isActive={oddEvent} 
                                          ...
                                          className={"submenu btn" + ((this.state.current == sublink.path)?" active":"")}
              ...
              <hr class="d-lg-none" />
              <form className="form-inline my-2 my-lg-0" style={{alignContent:'right',padding:"6px 12px"}}>
                ...
                <Dropdown isOpen={this.state.dropdownAccount} toggle={this.toggleAccount} size="sm" className="btn-dropdown-white">
                  ...
                  <DropdownMenu right>
                      ...
                      <NavLink to="/my-reported-issues" className="btn navlink-none">Reported Issues</NavLink>
